...
function PositionBadge({ position }: { position: number }) {
  ...
  return (
    <span className="text-lg font-bold text-muted-foreground w-8 text-center">{position}</span>
  );
}
...
function LeaderboardRow({
  ...
  return (
    <Link href={`/u/${entry.userId}`}>
      <div
        className={cn(
          "flex items-center gap-4 rounded-xl border p-4 transition-colors hover:border-primary/50 cursor-pointer",
      ...
      >
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="flex flex-col min-w-0">
            ...
            {entry.rank && (
              <Badge variant="outline" className="w-fit text-xs font-bold px-2 py-0 mt-0.5">
        ...
        <div className="hidden sm:flex items-center gap-6 text-sm shrink-0">
          ...
          <div className="text-center">
            <div className="font-bold text-foreground">{entry.piecesCount.toLocaleString()}</div>
            <div className="text-xs text-muted-foreground uppercase">Pieces</div>
          </div>
        </div>

        {/* Brick Score */}
        <div className="text-right min-w-[80px] shrink-0">
          <div className="text-lg font-black text-primary">{entry.brickScore.toLocaleString()}</div>
          <div className="text-xs text-muted-foreground uppercase">Brick Score</div>
        </div>
      </div>
    </Link>
  );
